...
def _make_simple_pdf(text: str = "Hello PDF world content") -> bytes:
    ...
    优先使用 reportlab；否则手工构造最小 PDF（含 xref）。
    """
    import io as _io
...
class TestDocumentTextService:
    ...
    def test_extract_text_from_pdf(self):
        """PDF 文件应提取出纯文本（与解析链一致支持 pdf）。"""
        service = DocumentTextService()
        pdf_bytes = _make_simple_pdf("Hello PDF world content")
        text = service._extract_text(pdf_bytes, "招标文件.pdf")
        assert "Hello PDF world content" in text

    def test_extract_text_from_pdf_corrupt(self):
        """损坏的 PDF 应抛出异常（由调用方记录任务失败原因）。"""
        service = DocumentTextService()
