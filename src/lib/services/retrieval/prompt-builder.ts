...
export class StructuredPromptBuilder {
  ...
  private buildSystemMessage(
    ...
    // 章节内部子标题应该比当前章节深一级
    const internalSubHeaderLevel = level + 1;
...
3. **准确性**: 只使用提供的参考资料，不编造数据或案例
4. **专业性**: 使用行业术语，避免空话套话，每项承诺需有依据
...
### 重要规则：
1. **不要输出章节主标题**！章节主标题"${sectionTitle}"会由系统自动添加，你只需要输出章节内容
2. 章节内部的子标题，请使用以下层级：

${this.getInternalHeaderFormat(internalSubHeaderLevel)}

### 标题层级对应关系：
- 当前章节是第${level}级，章节内部子标题应从第${internalSubHeaderLevel}级开始
- 例如：如果当前章节是"1.1 投标函正文"（二级），内部子标题应该用"###"开头
- 绝对禁止使用与当前章节同级的"##"标题

### 错误示例（禁止）：
\`\`\`
## 一、投标声明    ← 错误！这会与章节主标题冲突
## 二、报价承诺    ← 错误！这会与章节主标题冲突
\`\`\`
...
- 规避风险提示中的问题`;
   ...
   * 获取内部子标题格式说明
   */
  private getInternalHeaderFormat(level: number): string {
...
##### 1）[更细分的标题]（五级标题）`,
    ...
    return formats[level] || formats[3];
  }
